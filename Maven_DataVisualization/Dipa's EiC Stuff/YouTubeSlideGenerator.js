...
function ($, BaseSlideGenerator) {
           ...
           BaseSlideGenerator.prototype,
  {
    ...
    addVideoSlide: function (videoID, duration, thumbnail) {
      var self = this,
      ...
      var $placeholder = $('<div>').append("<img src=" + thumbnail + " />"),
      ...
      slide.once('started', function () {
        ...
        $placeholder.remove(); //remove the thumbnail img
    ...
    $.ajax('https://gdata.youtube.com/feeds/api/videos?v=2&max-results=' + maxResult + '&orderby=' + self.orderMethod + '&alt=jsonc&q=' + self.topic.label + "&format=5")
     .success(function (response) {
        ...
        for (var i = 0; i < itemCount; i++)
          self.addVideoSlide(items[i].id, items[i].duration * 1000, items[i].thumbnail.hqDefault);
      });
  }
